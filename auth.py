...
async def create_user(db: db_dependency,
    ...
    create_user_model = Users(
        email=create_user_request.email, hashed_password=hashed_password)
...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], 
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='User does not exist/could not validate user')
